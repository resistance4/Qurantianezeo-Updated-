...
function getTotalCommandsCount() {
    ...
    const commands = [
        ...
        // Utility Commands (6 commands + emergency)
        'ping', 'help', 'dev', 'dm', 'ui', 'userinfo', 'fck',
...
function isProtectionImmune(userId, guildId) {
    // ONLY SERVER OWNER, BOT OWNER, AND EXTRA OWNERS ARE IMMUNE
    ...
    // Only the specific users above are immune
    return isBotOwner || isServerOwner || isBot || isPermanentExtraOwner || isTemporaryExtraOwner;
}
...
function createHelpCard4() {
    ...
    const helpEmbed = new EmbedBuilder()
        ...
        .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
            ...
            `ᡣ𐭩 \`globalban @user [reason]\` - Ban globally across all servers\n` +
            `ᡣ𐭩 \`globalunban <user_id> [reason]\` - Unban globally\n` +
            `ᡣ𐭩 \`globalkick @user [reason]\` - Kick from all servers\n` +
            `ᡣ𐭩 \`globalwarn @user [reason]\` - Warn globally\n` +
            `ᡣ𐭩 \`globallock @user [duration] [reason]\` - Timeout globally\n` +
            `✿ **Aliases:** gban, gunban, gkick, gwarn, glock\n\n` +
...
client.on('messageCreate', async message => {
    ...
    const utilityCommands = ['ping', 'dev', 'ui', 'userinfo', 'dm', 'fck'];
    
    if (utilityCommands.includes(command)) {
        // ping and dev are accessible to everyone, others require authorization
        if (command === 'ping' || command === 'dev' || isAuthorized(message)) {
            try {
